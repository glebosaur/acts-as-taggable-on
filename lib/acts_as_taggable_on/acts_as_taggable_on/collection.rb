  ...
  module Collection
    ...
    module ClassMethods
        ...
        tag_types.map(&:to_s).each do |tag_type|
          class_eval <<-RUBY, __FILE__, __LINE__ + 1
            ...
            def top_#{tag_type}(limit = 10)
              tag_counts_on('#{tag_type}', :order => 'ccount desc', :limit => limit.to_i)
            end
      ...
      def all_tag_counts(options = {})
        ...
        tag_scope = ActsAsTaggableOn::Tag.select("#{ActsAsTaggableOn::Tag.table_name}.*, #{ActsAsTaggableOn::Tagging.table_name}.tags_count AS ccount").order(options[:order]).limit(options[:limit])
